...
class ProjectsController < ApplicationController
  ...
  def edit
    @category_instances = Category.all
    @category_names = @category_instances.map(&:name)
    @category_ids = @category_instances.map(&:id)
  end
  ...
  def project_params
    params.require(:project).permit(:title, :description, :url, :creation_date, :category_id, :photo)
  end
